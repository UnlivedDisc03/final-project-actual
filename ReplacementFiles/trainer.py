    ...
    def _do_train(self, world_size=1):
        ...
        while True:
            ...
            if RANK in {-1, 0}:
                ...
                #CUSTOM ADDITION JAKUB WOCHNIAK
                minimum_epochs = getattr(self.args, "minimum_epochs", 0) #creates a minimum epochs parameter, sets its default to 0
                if (epoch + 1) >= minimum_epochs:#if current epoch (starting from 0) + 1 is greater than or equal to minimum epochs 
                    self.stop |= self.stopper(epoch + 1, self.fitness) #early stops

                self.stop |= final_epoch #allows for code to always stop on final epoch
